odoo.define('pos_promotion', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        checking_can_apply_promotion: function (promotion_condition_items) {
            ...
            for (product_id in quantity_line_by_product_id) {
                for (var i=0; i < promotion_condition_items.length; i++) {
                    var promotion_item_condition = promotion_condition_items[i];
                    if (promotion_item_condition.product_id[0] == parseInt(product_id) && promotion_item_condition.minimum_quantity > quantity_line_by_product_id[product_id]) {
                        condition = true;
                    }
                }
            }
            ...
            if (check == true) {
                ...
                while (i < gifts.length) {
                    ...
                    if (product) {
                        ...
                        selected_line.promotion = true;
                        selected_line.promotion_reason = ' gift from: ' + promotion.name;
                        selected_line.trigger('change', selected_line);
                    }
                    i++;
                }
