...
class SaleOrderLine(models.Model):
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            renew_taxes = line.tax_id.compute_all(
                        ...
                        line.product_uom_qty, product=line.product_id,
                        partner=line.order_id.partner_shipping_id)
            register_tax_amount = taxes['total_included'] - \
                taxes['total_excluded']
            renew_tax_amount = renew_taxes['total_included'] - \
                renew_taxes['total_excluded']
    ...
    def create_po(self):
        ...
        """
        PurchaseOrder = self.env['purchase.order'].sudo().with_context(company_id=self.order_id.partner_id.company_id.id, force_company=self.order_id.partner_id.company_id.id)
        PurchaseOrderLine = self.env['purchase.order.line'].sudo().with_context(company_id=self.order_id.partner_id.company_id.id, force_company=self.order_id.partner_id.company_id.id)
    ...
    def create_service(self):
        SaleService = self.env['sale.service'].sudo().with_context(company_id=self.order_id.company_id.id, force_company=self.order_id.partner_id.company_id.id)
        for record in self:
            data = {
            ...
            if record.parent_product_id:  # addon
                ...
                if not old_services:
                    SaleService.with_context(force_company=self.order_id.partner_id.company_id.id).create(data)
                else:
                    old_services.write({
                        'so_line_id': record.id,
                        'sales_order_ids': [(4, record.order_id.id)]})
