...
class SaleService(models.Model):
    ...
    def _compute_invoice_count(self):
        for service in self:
            invoice_no = 0
            for order in service.sales_order_ids:
                invoice_no += order.invoice_count
            service.invoice_count = invoice_no
    ...
    def renew_sales_orders(self):
        ...
        for service in renew_service_ids:
            ...
            vals = {
                ...
                'time': service.product_id.billing_cycle,
        ...
        for index, order in enumerate(data):
            ...
            r = SaleOrder.with_context(force_company=data[order].get('company_id')).create(data[order])
            self._cr.commit()
            orders |= r
            try:
                orders.update_price()
