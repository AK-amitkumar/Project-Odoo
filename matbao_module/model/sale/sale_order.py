...
class SaleOrder(models.Model):
    ...
    # @api.multi
    # @api.depends('invoice_ids', 'invoice_ids.state', 'invoice_ids.payment_ids',
    #              'invoice_ids.payment_ids.state')
    ...
    def update_price(self):
        ...
        for order in self:
            ...
            for order_line in order_lines:
                ...
                for res in so_lines:
                    if res['oderlineid'] not in order.order_line.ids:
                        continue
                    ...
                    tax = self.env['account.tax'].search(
                        [('type_tax_use', '=', 'sale'),
                         ('amount', '=', res['tax'])], limit=1)
                    line.tax_id = tax
            order.order_line.write({'price_updated': True})
    ...
    def _prepare_invoice_line_vals(
            self, register_type, price_unit, taxes_amount=0,
            tax_ids=[(6, 0, [])], time=1):
        vals = {
                'time': time,
    ...
    def action_invoice_create(self, grouped=False, final=False):
        ...
        # invoice_ids = super(SaleOrder, self.sudo().with_context(company_id=self.env.user.company_id.id, force_company=self.env.user.company_id.id)).action_invoice_create()
        ...
        for invoice_line in invoice.invoice_line_ids:
            ...
            # renew_time = order_line.register_type == 'register' and \
            #     order_line.time - 1 or order_line.time
            # renew_time = order_line.register_type == 'register' and 1 or order_line.time
            if order_line.renew_taxed_price:
                # Renew /Transfer Taxed price
                vals = self._prepare_invoice_line_vals(
                        register_type,
                        order_line.renew_taxed_price,
            ...
            if order_line.renew_untaxed_price:
                ...
                is_copy = self._create_invoice_lines(
                    invoice_line, vals, is_copy)
            if not is_copy:
                invoice_line.write({'register_type': order_line.register_type,
                                    'price_unit': 0})
