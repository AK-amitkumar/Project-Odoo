...
class ExternalPO(models.AbstractModel):
    ...
    def get_po_by_state(self, type, is_active, state=False, register_type=False, date_from=False, date_to=False, limit=None, order='date_order'):
        ...
        # Check Register Type
        if register_type and register_type not in ('register', 'renew', 'transfer'):
            res.update({'code': 0, 'msg': 'Type could not empty and must be in `register`, `renew` and `transfer`'})
            return res
        ...
        args += ['|', ('retry_count', '=', 0), ('retry_count', '=', False)]
        _logger.info("\n--------------------------- %s -------------------------- " % args)
        # If arguments are ok
        try:
            ...
            for po in po_recs:
                ...
                if type == 1:
                    ...
                    if (not register_type or any(line.register_type == register_type for line in po.mapped('order_line'))) \
                            and categs and any(c.code and c.code.strip()[:1] == '.' and c.code.strip()[-3:] == '.vn' for c in categs):
    ...
    def update_po_company(self, name, account1, cus_code1, account2, cus_code2, account3, cus_code3, result, vendor_code, ip_hosting, ip_email, start_date, end_date, vals={}):
        ...
        if not service:
            res['msg'] = \
                "Service of purchase order '{}' is not found".format(name)
            return res
        # print vals, 99999999999999999, vals.get('price_unit')
        if not vals.get('price_unit'):
    ...
    def update_po_final(self, name, cus_code1, cus_code2, cus_code3, result, vendor_code, ip_hosting, ip_email, start_date, end_date, vals={}):
        ...
        if result:
            ...
            _logger.info('666666666666x55555xxxxxxxx8888888883888888xxxxxxxxx555555555xxxxxxxxxxxx666666')
            po_line._compute_amount()
            _logger.info('666666666666x55555xxxxxxxx8888888884888888xxxxxxxxx555555555xxxxxxxxxxxx666666')
            po._amount_all()
            _logger.info('666666666666x55555xxxxxxxx8888888885888888xxxxxxxxx555555555xxxxxxxxxxxx666666')
            po.button_confirm()
            _logger.info('666666666666x55555xxxxxxxx8888888886888888xxxxxxxxx555555555xxxxxxxxxxxx666666')
            po.button_done()
            _logger.info('666666666666x55555xxxxxxxx8888888888788888xxxxxxxxx555555555xxxxxxxxxxxx666666')
            ...
            if vals.get('price_unit', 0) > 0 or vals.get('price_unit1',0) > 0:
                ...
                bill_vals1 = dict(bill_vals)
                _logger.info('ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ %s ' % bill_vals1 )
                ...
                if vals.get('price_unit', 0) > 0:
                    try:
                        ...
                        _logger.info('ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ')
                        self._cr.rollback()
                ...
                try:
                    ...
                    for line in customer_invoice.invoice_line_ids:
                        ...
                        # print _logger.info('%s 666666666666666666 %s' % account_id, cus2_id.company_id.name)
                        if account_id:
                            line.write({
                    ...
                    try:
                        ...
                        for line in vendor_bill_ncc.invoice_line_ids:
                            account = AccountInvoiceLine.with_context(force_company=cus3_id.company_id.id).get_account_id(
                                line.product_id, 'in_invoice', line.register_type, line.invoice_line_tax_ids)
                            # print account, 4444444444444444
                            if account:
                                line.write({
                                    'account_id': type(account).__name__ == 'int' and account or account.id
                                })
                ...
                if vals.get('price_unit1', 0) > 0:
                    try:
                        # print 888888888, bill_vals1_ncc
                        ...
                        for line in vendor_bill1_ncc.invoice_line_ids:
                            account = AccountInvoiceLine.with_context(force_company=cus3_id.company_id.id).get_account_id(
                                line.product_id, 'in_invoice', line.register_type, line.invoice_line_tax_ids)
                            # print account, '*'*20, line.invoice_line_tax_ids, line.register_type
                            if account:
                                line.write({
                                    'account_id': type(account).__name__ == 'int' and account or account.id
                                })
