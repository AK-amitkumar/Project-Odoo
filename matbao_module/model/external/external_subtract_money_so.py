            ...
            if order.state == 'sale':
                ...
                if not order.invoice_ids or not any(inv.state <> 'cancel' for inv in order.invoice_ids):
                    try:
                        ...
                        return {'"code"': 0, '"msg"': '"Can`t create Invoice for SO %s"' % so_name}
                    ...
                    try:
                        _logger.info('validate invoice %s ------------22222222222222222-----------' % invoice_id.name)
                        invoice_id.with_context(force_company=order.partner_id.company_id.id).action_invoice_open()
                        _logger.info('validate invoice %s successfully ------------2222222222222222-----------' % invoice_id.name)
                    except:
                        return {'"code"': 0, '"msg"': '"Can`t validate Invoice (with SO %s)"' % so_name}
                elif any(inv.state == 'draft' for inv in order.invoice_ids):
                    try:
                        _logger.info('validate invoice %s ------------3333333333333333333333-----------' % invoice_id.name)
                        invoice_id = order.invoice_ids.filtered(lambda inv: inv.state == 'draft')
                        invoice_id.with_context(force_company=order.partner_id.company_id.id).action_invoice_open()
                        _logger.info('validate invoice %s successfully ------------3333333333333333333-----------' % invoice_id.name)
                    except:
                        return {'"code"': 0, '"msg"': '"Can`t validate Invoice (with SO %s)"' % so_name}
                ...
                try:
                    if invoice_id.outstanding_credits_debits_widget:
                        ...
                        _logger.info('Check money ------------444444444444444444444444----------- %s' % invoice_id.outstanding_credits_debits_widget)
                        if outstanding:
                    ...
                    return {'"code"': 0, '"msg"': '"Subtract Money Fail!"'}
            else:
