...
class ResPartner(models.Model):
    ...
    country_id = fields.Many2one(
        ...
        default=lambda self:
        self.env['res.country'].search([('name', '=', 'Vietnam')], limit=1))
    ...
    def _compute_payment_count(self):
        ...
        for r in self:
            payment_count = AccoutPayment.search_count([('partner_id', '=',
                                                         r.id)])
    ...
    def _compute_service_count(self):
        ...
        for r in self:
            service_count = SaleService.search_count([
                ('customer_id', '=', r.id)])
    ...
    def create(self, vals):
        ...
        if (vals.get('customer', False) or vals.get('supplier', False)) and not vals.get('ref', False):
            vals.update({
                'ref': IrSequence.next_by_code('res.partner') or '/',
            })
